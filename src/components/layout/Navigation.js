...
const Navigation = ({ currentPage, setCurrentPage, currentTime, isMenuOpen, setIsMenuOpen }) => (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {['Home', 'Categories', 'Trending', 'Digest', 'Analytics'].map((item) => (
            <button
              key={item}
              onClick={() => setCurrentPage(item.toLowerCase())}
              className={`text-sm font-medium transition-all duration-300 hover:text-purple-400 ${
                currentPage === item.toLowerCase() ? 'text-purple-400' : 'text-gray-300'
              }`}
            >
              {item}
            </button>
