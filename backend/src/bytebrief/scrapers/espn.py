...
class ESPNScraper(BaseScraper):
    ...
    def scrape(self) -> List[Article]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch ESPN RSS feed: {e}")
            return articles

        items = soup.find_all('item')[:10]
        logger.info(f"Found {len(items)} items in ESPN RSS feed")
        ...
        for item in items:
            try:
                ...
                article = Article(
                    ...
                    url=article_url,
                    source=source_config.get('name', 'ESPN'),
                    author=None,
                    published_date=None,
                    image_url=image_url,
                    category='Sports'  # Pre-tag sports source
                )
                articles.append(article)
                self._rate_limit(source_name)

            except Exception as e:
                logger.error(f"Error processing ESPN RSS item: {e}")
                continue
