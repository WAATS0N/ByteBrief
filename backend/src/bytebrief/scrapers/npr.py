...
class NPRScraper(BaseScraper):
    ...
    def scrape(self) -> List[Article]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch NPR RSS feed: {e}")
            return articles

        items = soup.find_all('item')[:10]
        logger.info(f"Found {len(items)} items in NPR RSS feed")
        ...
        for item in items:
            try:
                ...
                article = Article(
                    ...
                    url=article_url,
                    source=source_config.get('name', 'NPR'),
                    author=None,
                    published_date=None,
                    image_url=image_url
                )
                ...
                logger.error(f"Error processing NPR RSS item: {e}")
                continue
